...
================================================================================
 ...
 Supervisor-specified pipeline. Version 2 (self-tuning).
================================================================================
...
# CONFIG
# ==============================================================================
INPUT_PATH = r"S:\soscho\trajectories.csv"
OUTPUT_CSV = r"S:\soscho\trajectories_filtered.csv"
OUTPUT_TXT = r"S:\soscho\phase1_diagnostics.txt"
...
for vid, t in df.groupby("vehicle_id"):
    ...
    latv95 = float(np.nanpercentile(np.abs(t["_lat"].diff() / DT), 95))
    why = []
...
for vid, t in df.groupby("vehicle_id"):
    ...
    t["time"] = t.frame * DT
    lon_c, f1 = hampel(t["_lon"].to_numpy(), HW, HAMPEL_NSIGMA)
    lat_c, f2 = hampel(t["_lat"].to_numpy(), HW, HAMPEL_NSIGMA)
    n_lon += int(f1.sum()); n_lat += int(f2.sum()); n_tot += len(t)
    t["_lon_h"], t["_lat_h"] = lon_c, lat_c
    tracks.append(t.reset_index(drop=True))

say(f"Tracks retained  : {len(tracks)}   (dropped {dropped} below {MIN_TRACK_FRAMES} frames)")
...
for ti, t in enumerate(tracks):
    ...
    p, v, a = smooth_series(BEST, z, DT, PARAM, SIG_LON)
...
clean = pd.concat(out_rows, ignore_index=True).sort_values(["vehicle_id", "frame"])
...
clean["direction"] = np.where(
    clean.groupby("vehicle_id").speed_mps.transform("mean") >= 0, "with_flow", "opposing")

try:
    clean.to_csv(OUTPUT_CSV, index=False)
    say(f"\nCleaned file written : {OUTPUT_CSV}   ({len(clean)} rows, "
        f"{clean.vehicle_id.nunique()} tracks)")
...
say(f"dt={DT:.5f}s  fps={FPS:.2f}  tracks={clean.vehicle_id.nunique()}  rows={len(clean)}  "
...
say(f"implausible rows remaining: {int(clean.flag_implausible.sum())} "
    ...
    f"opposing-flow tracks: {(clean.groupby('vehicle_id').direction.first()=='opposing').sum()}")

wf = clean[clean.direction == "with_flow"]
say("\nPER-CLASS KINEMATICS, with-flow tracks only (EMPIRICAL, post-filter):")
say(f"{'class':<10}{'trk':>5}{'rows':>7}{'v_mean':>9}{'v_p85':>8}{'v_p95':>8}"
    f"{'a+_p95':>8}{'a-_p05':>8}{'|latv|p95':>10}{'lat_rng':>9}")
say("-" * 78)
for cn, t in wf.groupby("class_name"):
    v = t.speed_mps.to_numpy(); a = t.accel_mps2.to_numpy()
    lv = np.abs(t.lat_speed_mps.to_numpy())
    say(f"{str(cn):<10}{t.vehicle_id.nunique():>5}{len(t):>7}"
        ...
        f"{np.percentile(lv,95):>10.3f}{t.lat_m.max()-t.lat_m.min():>9.2f}")
say("  v in km/h | a in m/s^2 | latv in m/s | lat_rng = lateral span in m")
...
qt = clean.groupby("vehicle_id").apply(lambda t: pd.Series({
...
say("\nPHASE 2 READINESS BY CLASS (does each class have enough empirical support?):")
for cn, t in clean.groupby("class_name"):
    ntrk = t.vehicle_id.nunique()
    verdict = ("EMPIRICAL fit possible" if ntrk >= MIN_TRK_PER_CLS
               else "TOO THIN -> synthetic augmentation, to be declared")
    say(f"   {cn:<12s} {ntrk:>5d} tracks, {len(t)*DT:>8.1f} vehicle-seconds   {verdict}")
for c in [c for c in CLASS_SYNONYMS if c not in set(clean.class_name)]:
    say(f"   {c:<12s} {0:>5d} tracks                          ABSENT -> fully synthetic")
